// An statically allocated variable or array hasafixed size in memory.
// We have learnt to create big enough arrays to fit in our inputs but this doesn't seem like an optimal way to
//  allocate memory.
// Memory isavery useful resource.
// Clearly we needaway to request memory on runtime.
// Dynamic Memory Allocation isaway in which the size ofadata structure can be changed during the runtime.
