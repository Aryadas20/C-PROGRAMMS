/*The concept of the NULL pointer is very easy and simple to understand. A NULL pointer is a pointer that does not point to any memory location. It generally points to NULL or 0th memory location, so in simple words, no memory is allocated to a NULL pointer.
...
void *ptr;
Now, this is a void pointer in which the value will set according to what sort of value we store in it. 

If we equal it to an integer, then its value will be int, and if we pass a character variable in it then, its value will be char and so on. For further clarification about the difference, you may visit the previous tutorial i.e., tutorial #52. 
