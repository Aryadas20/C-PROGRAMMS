/*  We have already discussed the local and global variables in C in our previous tutorial. Today’s tutorial is mainly focused on the concept of static variables. As we know that the scope in any programming, whether it is C or Python, is a region of the program where a defined variable can exist and beyond which it cannot be accessed. For a quick recap, let us revise the concept of a local and global variable.
